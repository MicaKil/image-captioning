...
def gen_caption(model: nn.Module, images: torch.Tensor, vocab: Vocabulary, max_len: int = 30, device: torch.device = torch.device("cpu"), temp: Optional[float] = None,
                beam_size: int = 1, no_grad=True) -> tuple[list[str], Tensor]:
    """
    ...
    return model.generate(images=images, vocab=vocab, max_length=max_len, device=device, temperature=temp, beam_size=beam_size,
                          no_grad=no_grad)
