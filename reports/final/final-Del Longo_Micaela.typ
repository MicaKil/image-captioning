...
El repositorio del código fuente se encuentra disponible en GitHub a través del siguiente link https://github.com/MicaKil/image-captioning. Este mismo cuenta con dos aplicaciones CLI: una para correr el pipeline de entrenamiento y testeo, y otra para generar descripciones de imágenes aportadas por el usuario.
...
#figure(
image("compressed/histogram_caption_len_flickr8k.png"),
  caption: [Histograma de frecuencia del largo descripción de imagen en cada set del dataset Flickr8k.],
  placement: auto
) <hist-words-f8k>

#figure(
image("compressed/boxplot_caption_len_flickr8k.png", height: 13cm),
...
#figure(
  image("compressed/histogram_caption_len_coco.png"),
  caption: [Histograma de frecuencia de largo descripción de imagen en cada set del dataset COCO.],
  placement: auto
)<hist-words-coco>

#figure(
  image("compressed/boxplot_caption_len_coco.png", height: 13cm),
...
#figure(
  image("compressed/flickr8k_vocab_analysis.png"),
...
#figure(
  image("compressed/coco_vocab_analysis.png"),
...
=== Encoder Swin (Modelo FullAttention)
...
El Swin Transformer (Shifted Window Transformer) es un tipo de arquitectura transformers, diseñada especialmente para tareas de visión por computadora (como clasificación de imágenes, detección de objetos o segmentación). A diferencia de los Vision Transformers (ViT) originales, que procesan la imagen entera dividiéndola en parches y aplicando atención global, el Swin Transformer:

- Aplica atención dentro de ventanas locales dividiendo la imagen en bloques o ventanas pequeñas y aplica atención auto-regresiva solo dentro de esas ventanas. Esto reduce el costo computacional.

- Introduce ventanas desplazadas ("shifted windows"). Estas, en capas alternas, se desplazan un poco, lo cual permite que haya comunicación entre regiones vecinas, algo que se perdería si cada ventana siempre fuera fija. Como consecuencia, esto mejora la captura de contexto global sin usar atención global completa.

- Es jerárquico al ir reduciendo la resolución progresivamente para extraer representaciones a diferentes escalas. Lo cual es útil para detectar objetos de distintos tamaños, como por ejemplo.

La figura @swin-arq ilustra la arquitectura de este Transformer (a) y de los bloques de Swin Transformer (b). En esta figura:
-  W-MSA and SW-MSA son módulos multi-head self attention con ventanas regulares y deslizantes, respectivamente.
- LN es Layer Normalization.
- MLP es Multilayer Perceptron.
- $hat(z)^l$ y $z^l$ denotan la salida del módulo (S)W-MSA y del módulo MLP para el bloque $l$, respectivamente.

#figure(
  grid(
    columns: (1fr, 1fr),
    grid.cell(colspan: 2, image("figures/swin-arq.png")),
    sub_caption[(a)],
    sub_caption[(b)],
  ),
  caption: [Figuras de @swin[p.4]. (a) Arquitectura de Swin Transformer (Swin-T); (b) Dos bloques sucesivos de Swin Transformer.],
  placement: top
) <swin-arq>
...
Durante el paso hacia adelante (forward), la imagen de entrada es procesada primero por las capas modificas de Swin. Luego, el tensor de características obtenido se pasa a través de la capa de proyección, resultando en un tensor de características de salida con la dimensión de embedding especificada.
...
La @full-attn ilustra la arquitectura del modelo y el flujo de datos a través de este. En dicho diagrama cada color de bloque indica lo siguiente:
- Blanco: Entrada
- Gris: Salida
- Verde: Capa del Encoder
- Azul: Capa de Embedding Secuencial
...
#figure(
  image("figures/Swin-Attention.png"),
  caption: [Diagrama de la arquitectura y el flujo de datos en FullAttention.],
  placement: auto
) <full-attn>

Como se puede apreciar, la arquitectura es básicamente la misma que la anterior con la salvedad de que se utiliza el Swin Transformer como Encoder.
...
#figure(
  grid(
    columns: 1,
    row-gutter: 3pt,
    image("compressed/val_loss_ResNet50-LSTM_flickr8k.png", width: 100%),
    sub_caption[(a)],
    image("compressed/val_loss_ResNet50-Attention_flickr8k.png", width: 100%),
    sub_caption[(b)],
  ),
...
#figure(
  grid(
    columns: 2,
    row-gutter: 3pt,
    image("compressed/boxplot_val_loss.min_flickr8k.png", width: 8.5cm),
    image("compressed/boxplot_test_BLEU-4.max_flickr8k.png", width: 8.5cm),
    sub_caption[(a)],
    sub_caption[(b)],
    grid.cell(colspan: 2, image("compressed/boxplot_test_CIDEr.max_flickr8k.png", width: 8.5cm)),
...
#figure(
  grid(
    columns: 1,
    row-gutter: 2pt,
    image("compressed/ResNet50-LSTM_val_loss.min_correlation_importance.png"),
    sub_caption[(a)],
    image("compressed/ResNet50-LSTM_test_BLEU-4.max_correlation_importance.png"),
    sub_caption[(b)],
    image("compressed/ResNet50-LSTM_test_CIDEr.max_correlation_importance.png"),
    sub_caption[(c)],
  ),
...
#figure(
  grid(
    columns: 1,
    row-gutter: 2pt,
    image("compressed/ResNet50-Attention_val_loss.min_correlation_importance.png"),
    sub_caption[(a)],
    image("compressed/ResNet50-Attention_test_BLEU-4.max_correlation_importance.png"),
    sub_caption[(b)],
    image("compressed/ResNet50-Attention_test_CIDEr.max_correlation_importance.png"),
    sub_caption[(c)],
  ),
...
A modo ilustrativo, en la @flickr-captions se presentan ejemplos de descripciones generadas por estos modelos. Las descripciones fueron obtenidas utilizando beam search con tamaño 5, y se aplicaron sobre un conjunto de imágenes nunca vistas por el modelo.

#figure(
  grid(
    columns: 2,
    row-gutter: 3pt,
    image("compressed/flickr8k_001.png", width:7.5cm),
    image("compressed/flickr8k_002.png", width:7.5cm),
    image("compressed/flickr8k_003.png", width:7.5cm),
    // image("compressed/flickr8k_004.png", width:7.5cm),
    image("compressed/flickr8k_005.png", width:7.5cm),
    image("compressed/flickr8k_006.png", width:7.5cm),
    image("compressed/flickr8k_007.png", width:7.5cm),
  ),
  caption: [Descripciones generadas con los modelos ResNet50-LSTM y ResNet50-Attention entrenados en el dataset Flickr8k. Las primeras tres imágenes son de la autora, mientras el resto proviene de fuentes externas citadas a continuación de izquierda a derecha, arriba hacia abajo: (a) @img_farmers, (b) @img_kevin y (c) @img_kingfisher.],
  placement: auto,
) <flickr-captions>
...
#figure(
  image("compressed/val_loss_ResNet50-LSTM_ResNet50-Attention_coco.png"),
...
#figure(
  grid(
    columns: 3,
    row-gutter: 2pt,
    image("compressed/boxplot_val_loss.min_coco.png"),
    image("compressed/boxplot_test_CIDEr.max_coco.png"),
    image("compressed/boxplot_test_BLEU-4.max_coco.png"),
    sub_caption[(a)],
    sub_caption[(b)],
    sub_caption[(c)],
  ),
...
#figure(
  grid(
    columns: 2,
    row-gutter: 3pt,
    image("compressed/coco_001.png", width:7.5cm),
    image("compressed/coco_002.png", width:7.5cm),
    image("compressed/coco_003.png", width:7.5cm),
    // image("compressed/coco_004.png", width:7.5cm),
    image("compressed/coco_005.png", width:7.5cm),
    image("compressed/coco_006.png", width:7.5cm),
    image("compressed/coco_007.png", width:7.5cm),
  ),
  caption: [Descripciones generadas con los modelos ResNet50-LSTM y ResNet50-Attention entrenados en el dataset COCO. Las primeras tres imágenes son de la autora, mientras el resto proviene de fuentes externas citadas a continuación de izquierda a derecha, arriba hacia abajo: (a) @img_farmers, (b) @img_kevin y (c) @img_kingfisher.],
  placement: auto,
) <coco-captions>
...
#figure(
  image("compressed/val_loss_Swin-Attention_flickr8k_coco.png", width: 14cm),
...
#figure(
  grid(
    columns: 2,
    row-gutter: 3pt,
    image("compressed/swin_001.png", width:7.5cm),
    image("compressed/swin_002.png", width:7.5cm),
    image("compressed/swin_003.png", width:7.5cm),
    // image("compressed/swin_004.png", width:7.5cm),
    image("compressed/swin_005.png", width:7.5cm),
    image("compressed/swin_006.png", width:7.5cm),
    image("compressed/swin_007.png", width:7.5cm),
  ),
  caption: [Descripciones generadas con el modelo FullAttention sobre el dataset Flickr8k y sobre COCO. Las primeras tres imágenes son de la autora, mientras el resto proviene de fuentes externas citadas a continuación de izquierda a derecha, arriba hacia abajo: (a) @img_farmers, (b) @img_kevin y (c) @img_kingfisher.],
  placement: auto,
) <swin-captions>
...
#figure(
  grid(
    columns: 1,
    image("compressed/coco_t_attention_007.png"),
  ),
...
#figure(
  grid(
    columns: 1,
    image("compressed/swin_coco_attention_002.png")
  ),
...
//   table(
//     columns: 5,
//     [Modelo],	[Dataset],	[CIDEr],[	BLEU-4], 	[Pérdida],
//     [ResNet50-LSTM],	[Flickr8k],	[0.37],	[0.15], [2.60],
//     [ResNet50-Attention],	[Flickr8k],	[0.44],	[0.18], [2.62],
//     [FullAttention],	[Flickr8k],	[0.50],	[0.20], [2.42],
//     [ResNet50-LSTM],	[COCO],	[0.70],	[0.24], [2.35],
//     [ResNet50-Attention],	[COCO],	[0.73],	[0.26], [2.17],
//     [FullAttention],	[COCO],	[0.75],	[0.26], [2.17],
//   )
// )
...
#appendix[
  ...
  == Mapas de Atención
  #figure(
    image("compressed/coco_t_attention_001.png"),
    caption: [Mapas de atención generados por el modelo RestNet50-Attention entrenado con el dataset COCO.]
  )
  #figure(
    image("compressed/coco_t_attention_003.png"),
    caption: [Mapas de atención generados por el modelo RestNet50-Attention entrenado con el dataset COCO.]
  )
  #figure(
    image("compressed/swin_coco_attention_003.png"),
